...
TEST(Reflection, MemberFunctions)
{
    ...
    i = refl::callFunction<int(int)>(IHaveAReflMemFunc::reflectedFunctions(), "func", iharf2, 3);
    ASSERT_EQ(i, 3);

    ASSERT_THROW(refl::callFunction<int()>(IHaveAReflMemFunc::reflectedFunctions(), "lala", iharf2),
                 refl::MemberNotFound);
}

...
#define reflect_class(cls)                                                  \
    ...
    template <typename MapT, typename KeyT, typename Function>
    static inline bool constexpr getFunction(MapT &&map, KeyT &&k, Function &&func) noexcept
    {
    ...
    template <typename MemberPtrT, typename MapT, typename KeyT, typename Obj, typename ...Args>
    static inline auto callFunction(MapT &&map, KeyT &&k, Obj &&obj, Args &&...args)
    {
        using ObjRawType = std::decay_t<std::remove_pointer_t<std::decay_t<Obj>>>;
        using FunctionT = MemberPtrT ObjRawType::*;
        auto memberOpt = getMember<FunctionT>(std::forward<MapT>(map), std::forward<KeyT>(k));

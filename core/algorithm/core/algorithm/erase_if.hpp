...
{
    template <typename T>
    using check_key_type = decltype(std::declval<typename T::key_type>());

    template <typename T>
    using check_value_type = decltype(std::declval<typename T::value_type>());

    template <typename T>
    using is_associative_container = std::conjunction<meta::is_detected<check_key_type, T>, meta::is_detected<check_value_type, T>>;

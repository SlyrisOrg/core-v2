...
{
    template<typename T>
    using associative_compare_t = decltype(std::declval<T &>().emplace(std::declval<const typename T::key_type &>()));
...
{
    template <typename T>
    static inline constexpr bool is_associative_container_v = details::is_associative_container<T>::value;
